# Problem Set 5: Dynamic Discrete Choice Model of Bus Engine Replacement
...
function load_static_data()
    ...
    # we need to transorm the dataset so that it passes through GLM 
    ...
    # DatsFranes.stack() reshapes the data from wide to long format and create a new variable called Time
    # which indicates the time period (1 to 20) for each observation.
    ...
    df_long = leftjoin(dfy_long, dfx_long, on = [:bus_id, :time]) # join on bus_id and time
    sort!(df_long, [:bus_id, :time]) # sort by bus_id and time for easier viewing
...
function load_dynamic_data()
    ...
    # Instead of passing 10+ separate arguments to functions, we pass one named tuple
             ...
             for z in 1:d.zbin
                ...
                # Loop over mileage states  
                 for x in 1:d.xbin
                     ...
                     FV[row, b+1, t] =  d.β * log(exp(v1) + exp(v0)) + Base.MathConstants.eulergamma
                     # we add euler gamma because of the extreme value distribution of the error term
...
@views @inbounds function log_likelihood_dynamic(θ, d)
    ...
    FV = zeros(d.zbin * d.xbin, 2, d.T + 1) # d.xbin * d.zbin rows, 2 brand states, T+1 time periods
    # the product of d.xbin and d.zbin gives the total number of joint states (z,x)
    ...
    #Loop over individuals
     for i in 1:d.N
         ...
         row0 = (d.Zstate[i] - 1) * d.xbin + 1 # the first mileage bin for the given route usage state
        ...
        # Loop over time periods
         for t in 1:d.T
             ...
             flow_diff = θ[1] + θ[2]*d.X[i,t] + θ[3]*d.B[i] # d.X is the odometer reading for mileage, not the state and the 
             # other is the brand indicator
             ...
             # what is happening is that we are taking the difference of the two expected future values
             

             # Total conditional value difference
             v_diff =  flow_diff + d.β * ev_diff # don't need \beta * euler gamma here because the v's are differenced 
             ...
             # we do not need dots here because everything is a scalar
             
         end # end the loop over t 
     end # end the loop over i
...
"""
function estimate_dynamic_model(d; θ_start=nothing) # things after ; are keyword arguments or optional arguments
    println("="^70)
