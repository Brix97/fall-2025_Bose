include("create_grids.jl") # computation of transition probabilities
...
function estimate_flexible_logit(df::DataFrame)
    # Create squared terms for continuous variables- why? because we want to capture non-linear effects
...
"""
# getting different values of x and z to get the transition probabilities
function construct_state_space(xbin::Int, zbin::Int, xval::Vector, zval::Vector, xtran::Matrix)
    state_df = DataFrame(
        Odometer = kron(ones(zbin), xval), # xbin * zbin rows: why? because for each xval we have zbin values of zval
        RouteUsage = kron(zval, ones(xbin)),
        Branded = zeros(size(xtran,1)), # will fill in later
        time = zeros(size(xtran,1))  # will fill in later
    )
    return state_df
end

# Compute future values for all states, brands, and time periods
...
function compute_future_values(state_df::DataFrame, 
    ...
    FV = zeros(xbin*zbin, 2, T+1) # 2 is for the branded dummy and T+1 is for time periods including t=0
    ...
    #  Nested loops over time and brand
     for t in 2:T
         for b in 0:1
             ...
             p0 = 1 .- convert(Array{Float64}, predict(flex_logit, state_df)) # probability of replacement
             FV[:, b+1, t] = -β * log.(p0)  # store future value # Store -β * log.(p0) in FV
         end
     end
...
"""
# in equation 3 of the PDF we are calling the offset as the beta times the sum log part of the equation
...
function main()
    ...
    # Define grid parameters (xbin, zbin, xval, zval)
