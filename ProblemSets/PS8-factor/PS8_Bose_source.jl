...
# Hint
- You have many aspects - mathematical reasoning, coding skills, etc. 
...
"""
    generate_pca_regression(df::DataFrame)
...
function generate_pca(df::DataFrame)
    ...
    # Transform data to get principal component
        asvabPCA = MultivariatStats.transform(M, asvabs)
...
function generate_factor(df::DataFrame)
    ...
    # Transform data to get factor scores
        asvabFactor = MultivariatStats.transform(M, asvabs) # transform data to get factors
...
==================================================================================#
include("lgwt.jl") # include quadrature function
"""
...
# we have got all alphas, betas, gammas, sigmas in a vector theta and the variance of the error terms
# for the likelihood function we need to integrate out the latent factor using quadrature
...
function main()
    ...
    println("="^80)
    # estimate regression model of wage on demographics
    ...
    println("\nMulticollinearity is a problem; it doesn't make sense to 'hold all else equal' if
     the focal variable is highly correlated with the other variables in the model. They may all
     be derived from some latent common factor (a proxy), which is what we should be exogenously varying.")
